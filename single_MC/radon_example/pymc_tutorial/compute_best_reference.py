...
FOCAL_PARAMETERS = ["mu_a", "beta", "sigma_a_log__", "sd_y_log__"]
...
def _make_radon_fixed_guide(mean, sd, num_counties):
    beta_idx = 2 + num_counties
    return _make_fixed_diagonal_guide(
        mean,
        sd,
        [
            {"name": "mu_a", "slice": 0},
            {"name": "sigma_a", "slice": 1, "positive": True},
            {"name": "alpha", "slice": slice(2, beta_idx), "event_dim": 1},
            {"name": "beta", "slice": beta_idx},
            {"name": "sd_y", "slice": beta_idx + 1, "positive": True},
        ],
    )
...
def _radon_numpyro_score(mean, std, data, grad_samps, seed, **_kwargs):
    import jax
    from numpyro.infer import TraceMeanField_ELBO

    guide = _make_radon_fixed_guide(mean, std, data["num_counties"])
        ...
        rows.append(
            {
                ...
                "source": source,
                "model": "pymc_tutorial",
                "scale": "unconstrained_for_positive_scales",
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Compute a shared variational reference for the PyMC-tutorial radon model "
    ...
    param_names = make_radon_unconstrained_param_names(data["county_names"])
